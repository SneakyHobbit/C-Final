...
bool magicSpell(Player* pP, Room* pR, std::vector<std::string>& msgQ);
...
* add messages to caller's vector for display
*
* returns 'q' when Player is defeated and all lives are gone, or 'c'
* return value 'q' forces main loop to terminate
*******************************************************************************
*/
char incinerate(Player* pP, Room* pR, std::vector<std::string>& msgQ)
{
    if (!magicSpell(pP, pR, msgQ))
    {
        // get Player name
        std::string str;
        pP->getName(str);
...
bool magicSpell(Player* pP, Room* pR, std::vector<std::string>& msgQ)
{
    ...
    // get last MagicWord stored in vector
    std::vector<MagicWord*> mwV;
    pP->getMagicWordInfo(mwV);
    MagicWord* mpW = mwV.back();
    int wPower = mpW->getPower();
    ...
    // if MagicSpell more powerful than Bogie award points
    int mwPower = mpW->getPower();
    if (mwPower >= bPower)
    {
        int bPoints = pB->getPoints();
    ...
    // MagicWord weaker than Bogie lives to fight again
    if (mwPower > 0)
    {
        msgQ.push_back("\nYou missed the " + bName + "!");
        pB->setPower(bPower - mwPower);
        return true;
    }
...
*******************************************************************************
*/
Room* movePlayer(Player* pP, Room* pR, int direction, std::vector<std::string>& msgQ, bool teleport)
{
    ...
    if (!teleport)
        pNextRoom = pR->getDirectionPtr(direction);
